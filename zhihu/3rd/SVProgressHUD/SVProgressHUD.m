...
@property (nonatomic, strong) UIView * promptView;  //临时增加自定义弹窗
...
+ (void)showErrorWithStatus:(NSString*)status withBlockWord: (NSString *)sureWord
{
    [self showImage:[self sharedView].errorImage status:status sureWord: (NSString *)sureWord];
}

+ (void)showImage:(UIImage*)image status:(NSString*)status sureWord: (NSString *)sureWord {
    NSTimeInterval displayInterval = [self displayDurationForString:status];
    [[self sharedView] showImage:image status:status duration:displayInterval withSureWord:sureWord];
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        /**
         *  2016.4.11李佳更改       注释后为原版
         */
        _defaultMaskType = SVProgressHUDMaskTypeBlack; //SVProgressHUDMaskTypeNone;
        /**
         *  更改结束
         */
         ...
         *  李佳修改        注释为原文
         */
        _defaultAnimationType = SVProgressHUDAnimationTypeNative;       // SVProgressHUDAnimationTypeFlat;
        ...
        if ([[UIImage class] instancesRespondToSelector:@selector(imageWithRenderingMode:)]) {
            ...
            _successImage =  successImage;//[successImage imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
            _errorImage = errorImage;//[errorImage imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        } else {
        ...
        _cornerRadius = 10;// 14.0f;
        
        _minimumDismissTimeInterval = 1;// 5.0;
...
- (void)updateMask {
    ...
    switch (self.defaultMaskType) {
        ...
        case SVProgressHUDMaskTypeBlack:{
            ...
            self.backgroundLayer.backgroundColor = self.defaultMaskType == SVProgressHUDMaskTypeCustom ? self.backgroundLayerColor.CGColor :UIColorFromRGBA(0x000000, .15).CGColor;
...
// 自定义的弹出警告
- (void)updateViewHierachyWithSureWord: (NSString *)sureWord {
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            /**
             *  李佳修改2016.4.12   因为更改了提示文字，故将渲染去除
             */
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration withSureWord: (NSString *)sureWord{
    __weak SVProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong SVProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
            // Update / Check view hierachy to ensure the HUD is visible
            [strongSelf updateViewHierachyWithSureWord:sureWord];
...
#pragma clang diagnostic pop
            ...
            // Update values on subviews
            self.hudView.bounds = CGRectMake(0.0f, 0.0f, MAX(self.minimumSize.width, hudWidth) * 1.2, MAX(self.minimumSize.height, hudHeight) + 33);
            
            //添加底部的按钮
            UIButton *sureButton = [UIButton buttonWithType:UIButtonTypeCustom];
            sureButton.backgroundColor = [UIColor whiteColor];
            sureButton.frame = CGRectMake(0.0f, MAX(self.minimumSize.height, hudHeight), MAX(self.minimumSize.width, hudWidth) * 1.2, 33);
            [sureButton setTitle:sureWord forState:UIControlStateNormal];
            [sureButton setTitleColor: [UIColor whiteColor] forState:UIControlStateNormal];
            [sureButton.titleLabel setFont:[UIFont systemFontOfSize:14]];
            [sureButton addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(dismiss)]];
            self.userInteractionEnabled = YES;
            [self.hudView addSubview:sureButton];
            ...
            statusRect.size.width = self.hudView.bounds.size.width;
...
#pragma mark - 临时增加自定义弹窗
+ (void)showIWStautus:(NSString *)title
{
    [[SVProgressHUD sharedView] showIWConfigStatusWith:title];
}


- (void)showIWConfigStatusWith:(NSString *)title
{
    ...
    CGRect frame = [title boundingRectWithSize:CGSizeMake(200, CGFLOAT_MAX) options:NSStringDrawingTruncatesLastVisibleLine |NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:15.0]} context:nil];
    
    if (self.promptView) {
        UILabel *label = (UILabel *)[self.promptView viewWithTag:100];
        label.text = title;
        self.promptView.frame = CGRectMake(0, 0, frame.size.width+40, frame.size.height+30);
        self.promptView.center = CGPointMake(DEVICE_WIDTH/2, DEVICE_HEIGHT/2);
        label.frame = CGRectMake(0, 0, frame.size.width, frame.size.height);
        label.center = CGPointMake(self.promptView.frame.size.width/2, self.promptView.frame.size.height/2);
    }else{
        self.promptView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width+40, frame.size.height+30)];
        self.promptView.center = CGPointMake(DEVICE_WIDTH/2, DEVICE_HEIGHT/2);
        self.promptView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.8];
        ...
        UILabel * promptLabel = [UILabel createLabelWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height) font:[UIFont systemFontOfSize:15] textColor:UIColorFromRGB(0xffffff)];
        promptLabel.textAlignment = NSTextAlignmentCenter;
        promptLabel.text = title;
        promptLabel.center = CGPointMake(self.promptView.frame.size.width/2, self.promptView.frame.size.height/2);
        promptLabel.tag = 100;
        promptLabel.numberOfLines = 2;
        [self.promptView addSubview:promptLabel];
    }
    [self performSelector:@selector(removePromptView) withObject: nil afterDelay:1.6f];
}
